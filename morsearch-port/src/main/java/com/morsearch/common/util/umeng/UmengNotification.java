...
public abstract class UmengNotification {
	...
	public String send() throws Exception {
        ...
        if (status == 200) {
			...
			String msg="";
			if (jsonObj.getJSONObject("data").has("msg_id")){ /*android返回*/
			msg=jsonObj.getJSONObject("data").get("msg_id").toString();
			};
			if (jsonObj.getJSONObject("data").has("task_id")){ /*ios返回*/
				msg=jsonObj.getJSONObject("data").get("task_id").toString();
			};
			return msg;
