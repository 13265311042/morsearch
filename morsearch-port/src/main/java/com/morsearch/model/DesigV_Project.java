package com.morsearch.model;

import java.util.Date;

public class DesigV_Project {
    private Long sp_id;

    private String sp_no;

    private String sp_title;

    private String sp_citycode;

    private String sp_cityname;

    private Byte sp_signcode;

    private String sp_signname;

    private Integer sp_count_collect;

    private Integer sp_count_browse;

    private Boolean sp_flag_auditread;

    private Boolean sp_flag_adminread;

    private String sp_adminremark;

    private Long sp_createid;

    private String sp_createno;

    private Date sp_createdate;

    private Long sp_confirmid;

    private String sp_confirmno;

    private Date sp_confirmdate;

    private Byte sp_status;

    private String statusname;

    public Long getSp_id() {
        return sp_id;
    }

    public void setSp_id(Long sp_id) {
        this.sp_id = sp_id;
    }

    public String getSp_no() {
        return sp_no;
    }

    public void setSp_no(String sp_no) {
        this.sp_no = sp_no;
    }

    public String getSp_title() {
        return sp_title;
    }

    public void setSp_title(String sp_title) {
        this.sp_title = sp_title;
    }

    public String getSp_citycode() {
        return sp_citycode;
    }

    public void setSp_citycode(String sp_citycode) {
        this.sp_citycode = sp_citycode;
    }

    public String getSp_cityname() {
        return sp_cityname;
    }

    public void setSp_cityname(String sp_cityname) {
        this.sp_cityname = sp_cityname;
    }

    public Byte getSp_signcode() {
        return sp_signcode;
    }

    public void setSp_signcode(Byte sp_signcode) {
        this.sp_signcode = sp_signcode;
    }

    public String getSp_signname() {
        return sp_signname;
    }

    public void setSp_signname(String sp_signname) {
        this.sp_signname = sp_signname;
    }

    public Integer getSp_count_collect() {
        return sp_count_collect;
    }

    public void setSp_count_collect(Integer sp_count_collect) {
        this.sp_count_collect = sp_count_collect;
    }

    public Integer getSp_count_browse() {
        return sp_count_browse;
    }

    public void setSp_count_browse(Integer sp_count_browse) {
        this.sp_count_browse = sp_count_browse;
    }

    public Boolean getSp_flag_auditread() {
        return sp_flag_auditread;
    }

    public void setSp_flag_auditread(Boolean sp_flag_auditread) {
        this.sp_flag_auditread = sp_flag_auditread;
    }

    public Boolean getSp_flag_adminread() {
        return sp_flag_adminread;
    }

    public void setSp_flag_adminread(Boolean sp_flag_adminread) {
        this.sp_flag_adminread = sp_flag_adminread;
    }

    public String getSp_adminremark() {
        return sp_adminremark;
    }

    public void setSp_adminremark(String sp_adminremark) {
        this.sp_adminremark = sp_adminremark;
    }

    public Long getSp_createid() {
        return sp_createid;
    }

    public void setSp_createid(Long sp_createid) {
        this.sp_createid = sp_createid;
    }

    public String getSp_createno() {
        return sp_createno;
    }

    public void setSp_createno(String sp_createno) {
        this.sp_createno = sp_createno;
    }

    public Date getSp_createdate() {
        return sp_createdate;
    }

    public void setSp_createdate(Date sp_createdate) {
        this.sp_createdate = sp_createdate;
    }

    public Long getSp_confirmid() {
        return sp_confirmid;
    }

    public void setSp_confirmid(Long sp_confirmid) {
        this.sp_confirmid = sp_confirmid;
    }

    public String getSp_confirmno() {
        return sp_confirmno;
    }

    public void setSp_confirmno(String sp_confirmno) {
        this.sp_confirmno = sp_confirmno;
    }

    public Date getSp_confirmdate() {
        return sp_confirmdate;
    }

    public void setSp_confirmdate(Date sp_confirmdate) {
        this.sp_confirmdate = sp_confirmdate;
    }

    public Byte getSp_status() {
        return sp_status;
    }

    public void setSp_status(Byte sp_status) {
        this.sp_status = sp_status;
    }

    public String getStatusname() {
        return statusname;
    }

    public void setStatusname(String statusname) {
        this.statusname = statusname;
    }
}